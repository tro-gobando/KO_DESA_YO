...
SELECT DISTINCT
    ...
    CASE WHEN ea1.werks = '8C1F' OR ea1.werks = '8C2F'
         ...
         ELSE 'Centro'
    END                                                         AS zona,
    CASE WHEN ea1.kostl = t23.kostl AND ea1.trfar = t23.trfar
            THEN t23.gerencia1
         WHEN ea1.kostl = t23null.kostl
...
LEFT JOIN db_koandina_regional_analytics.hrposition_text_analytics pos
       ON pos.key1 = ea1.plans AND
          ...
          to_date(pos.dateto, "yyyy-MM-dd") >= ea1.fecha AND
          to_date(pos.datefrom, "yyyy-MM-dd") <= ea1.fecha
...
LEFT JOIN db_koandina_regional_analytics.hrorgunittext_analytics org
       ON org.key1 = ea1.orgeh AND
           to_date(org.datefrom, "yyyy-MM-dd") <= ea1.fecha AND
           to_date(org.dateto, "yyyy-MM-dd") >= ea1.fecha

WHERE --year(ea1.fecha) >= year(current_date)-2
year(ea1.fecha) = year(current_date)-1 and
month(ea1.fecha) < month(current_date)